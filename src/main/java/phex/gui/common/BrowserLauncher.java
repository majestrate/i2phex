 ...
 * 
 *  --- CVS Information ---
 *  $Id: BrowserLauncher.java 4363 2009-01-16 10:37:30Z gregork $
 */
...
public class BrowserLauncher {
    ...
    public static void openURL(String url) throws IOException {
        ...
        // First see if the user has defined a method for handling html.
        // If so, prefer this to auto-detection
        String htmlViewer =  phex.utils.Executer.getViewCommand( "html" , false);
        if ( htmlViewer != null )
        {
            Thread thread = new Thread ( ThreadTracking.rootThreadGroup,
                new Executer ( new URL(url), htmlViewer ) );
        ...
        switch (jvm) {
            ...
            case WINDOWS_NT:
                try
                {
                    WindowsShellExecute.executeViaShell( url );
                }
                catch ( UnsatisfiedLinkError exp )
                {
                    throw new IOException("UnsatisfiedLinkError: " + exp.getMessage() );
                }
                break;
            case WINDOWS_9x:
