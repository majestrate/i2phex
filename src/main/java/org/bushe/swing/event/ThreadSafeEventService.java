...
//         }
   ...
   private void checkTimeLimit(long start, Object event, EventSubscriber subscriber, VetoEventListener l) {
      ...
      if (end - start > timeThresholdForEventTimingEventPublication.longValue()) {
         publish(new SubscriberTimingEvent(this, Long.valueOf(start), Long.valueOf(end), timeThresholdForEventTimingEventPublication, event, subscriber, l));
      }
   }
   ...
   public void setCacheSizeForEventClass(Class eventClass, int cacheSize) {
      synchronized (cacheLock) {
         ...
         rawCacheSizesForEventClass.put(eventClass, Integer.valueOf(cacheSize));
   ...
   public void setCacheSizeForTopic(Pattern pattern, int cacheSize) {
      synchronized (cacheLock) {
         ...
         rawCacheSizesForPattern.put(pattern, Integer.valueOf(cacheSize));
...
//         System.out.println(weakRefPlusProxySubscriberCount);
...
//            System.out.println("Scheduling cleanup:"+new Date());
   ...
   class CleanupTimerTask extends TimerTask {
      ...
      public void run() {
         synchronized(listenerLock) {
//            System.out.println("Running cleanup:"+new Date());
